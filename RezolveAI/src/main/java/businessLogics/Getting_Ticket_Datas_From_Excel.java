...
@Component("LIC7818_PJT1001_PE_NLPc3aaa275-e4fb-452c-9be3-7ed6d25c4fce")
public class Getting_Ticket_Datas_From_Excel implements Nlp {
	@InputParams({ @InputParam(name = "TicketNo", type = "java.lang.String"),
		           @InputParam(name = "FilePath", type = "java.lang.String"),
		           @InputParam(name = "SheetName", type = "java.lang.String")})
	@ReturnType(name = "Value", type = "java.util.ArrayList")
	...
	public NlpResponseModel execute(NlpRequestModel nlpRequestModel) throws NlpException {
			...
			for (int i = 0; i < sheets; i++) {
				if (workbook.getSheetName(i).equalsIgnoreCase(SheetName)) {
					...
					while (cell.hasNext()) {
						...
						if (value.getStringCellValue().equalsIgnoreCase("TicketNo")) {
							coloumn = k;
						}
					...
					while (rows.hasNext()) {
						Row r = rows.next();
						if (r.getCell(coloumn).getStringCellValue().equalsIgnoreCase(TicketNo)) {
		...
		nlpResponseModel.getAttributes().put("Value", values);
